...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!id || typeof id !== "string" || !id.trim()) {
      ...
      id = generateSlug(titleForSlug, "sketch");
      if (!id || /^sketch-\d+$/.test(id)) {
        return NextResponse.json(
          { error: "Slug（英文，必填）：請填寫或先填寫英文標題以自動產生" },
          { status: 400 }
        );
      }
      id = await ensureUniqueSlug(db, "sketches", id);
    } else {
      id = id.trim().toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "") || "";
      if (!id) {
        return NextResponse.json(
          { error: "Slug（英文，必填）：請填寫有效的英文網址，例如 my-sketch" },
          { status: 400 }
        );
      }
      id = await ensureUniqueSlug(db, "sketches", id);
    }
    const count = await db.collection("sketches").countDocuments();
    const doc = {
      id,
      title: title || {},
    ...
    await db.collection("sketches").insertOne(doc);
    return NextResponse.json({
      ...doc,
