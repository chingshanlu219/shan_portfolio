...
export default function PhotographyDetailPage() {
  ...
  const [item, setItem] = useState<Photography | null>(null);
  ...
  return (
    <div className="min-h-screen py-12">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <p className="text-gray-600 mb-8">{getLocalized(item.description)}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {images.map((url, i) => (
                ...
                >
                  <MediaDisplay
                    url={url}
                    alt={`${getLocalized(item.title)} ${i + 1}`}
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: getLocalized(item.content) || "<p>No content.</p>",
          }}
        />
      </div>
    </div>
